...
@triton.jit
def _q5_0_gemv_fwd(x_ptr, qs_ptr, d_ptr, out_ptr, K, NB,
                   BLOCK_N: tl.constexpr):
    # out[n] = sum_b d[n,b] * (sum_k q5[n,k]*x[k] - 16*sum_k x[k])
    # q5: int8 (N, K) already unpacked to 0..31; d: fp32 (N, NB=K/32).
    pid = tl.program_id(0)
    offs_n = pid * BLOCK_N + tl.arange(0, BLOCK_N)
    acc = tl.zeros((BLOCK_N,), tl.float32)
    for b in range(NB):
        ...
        d = tl.load(d_ptr + offs_n * NB + b)                          # (BLOCK_N,)
        sx = tl.sum(xv, axis=0)
        dot = tl.sum(qs.to(tl.float32) * xv[None, :], axis=1)
        acc += d * (dot - 16.0 * sx)
    tl.store(out_ptr + offs_n, acc)


def q5_0_gemv(x: torch.Tensor, qs: torch.Tensor, d: torch.Tensor) -> torch.Tensor:
    """Q5_0 quantized GEMV: out = x @ W.T where W is Q5_0 (qs int8 (N,K) 0..31, d fp32 (N,K/32))."""
    x = x.contiguous().float()
    qs = qs.contiguous()
    d = d.contiguous()
    K = x.numel()
